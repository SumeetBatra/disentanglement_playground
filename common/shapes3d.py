...
def get_datasets(config):
    ...
    val_set = train_set.take(config['num_val_data'])

    train_set = train_set.shuffle(100000, seed=config['seed']).repeat().map(prepare_data, num_parallel_calls=tf.data.AUTOTUNE).batch(config['batch_size']).prefetch(buffer_size=tf.data.AUTOTUNE)
    val_set = val_set.map(prepare_data, num_parallel_calls=tf.data.AUTOTUNE).batch(config['batch_size']).prefetch(buffer_size=tf.data.AUTOTUNE)
